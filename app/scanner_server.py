...
async function api(path,options={}){options.headers={...(options.headers||{}),'X-CSRF-Token':csrf};const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),requestTimeoutMs);let response;try{response=await fetch(path,{...options,signal:controller.signal})}catch(_error){const error=new Error('Desktop did not respond. Check Wi-Fi, keep the scanner running, and try again.');error.network=true;throw error}finally{clearTimeout(timeout)}const data=await response.json().catch(()=>({}));if(!response.ok){const error=new Error(data.error||`Request failed (${response.status})`);error.status=response.status;throw error}return data}
...
class ScannerServer:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            self._server = None
            raise OSError("Could not open a local network port for the phone scanner.") from exc
